#!/usr/bin/env bash
# Single-GPU, non-SLURM launcher for pirlnav IL on the one-scene MP3D subset.
#
# Usage:
#   bash scripts/run_il_mp3d_1scene.sh                # smoke (200 updates, 2 envs)
#   bash scripts/run_il_mp3d_1scene.sh --full         # 20k updates, 4 envs
#   NUM_UPDATES=1000 NUM_ENVIRONMENTS=2 bash scripts/run_il_mp3d_1scene.sh
set -euo pipefail
...
TAG="${TAG:-mp3d_1scene_6cat_smoke}"
...
echo "[run_il_mp3d_1scene] mode=${MODE} updates=${NUM_UPDATES} envs=${NUM_ENVIRONMENTS} ckpts=${NUM_CHECKPOINTS}"
echo "[run_il_mp3d_1scene] tb=${TENSORBOARD_DIR}"
echo "[run_il_mp3d_1scene] ckpt=${CHECKPOINT_DIR}"
