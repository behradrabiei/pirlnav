...
# at 252x252 in pose-replay mode, plus the *oracle* egocentric object cloud
# (ground-truth per-scene cloud loaded from npz, no online depth/semantic
# accumulation).
#
# Drop-in successor to run_il_mp3d_1scene_dinov2_oracle_object_cloud.sh: same
# policy inputs (cached DINOv2 + oracle NPZ object cloud) but uses the
...
#   - EGO_OBJECT_CLOUD (oracle NPZ per scene; no online depth/semantic)
...
#
# Prereq 2 -- oracle object clouds (one-time per scene):
#   data/object_clouds/mp3d/<scene>/<scene>.npz  (dump_scene_object_clouds.py)
#
#   python dump_scene_object_clouds.py --output-dir data/object_clouds/mp3d
#
# Usage:
#   bash scripts/run_il_mp3d_1scene_dinov2_cached_252_oracle_object_cloud.sh                # smoke (200 updates, 2 envs)
#   bash scripts/run_il_mp3d_1scene_dinov2_cached_252_oracle_object_cloud.sh --full         # 20k updates, 4 envs
#   NUM_UPDATES=1000 NUM_ENVIRONMENTS=2 \
#       bash scripts/run_il_mp3d_1scene_dinov2_cached_252_oracle_object_cloud.sh
#
# Env-var overrides:
...
CONFIG="configs/experiments/il_objectnav_mp3d_dinov2_oracle_object_cloud.yaml"
TAG="${TAG:-mp3d_1scene_6cat_dinov2_cached_252_oracle_object_cloud}"
TENSORBOARD_DIR="tb/objectnav_il/${TAG}/"
CHECKPOINT_DIR="data/new_checkpoints_dinov2_cached_252_oracle_object_cloud/objectnav_il/${TAG}/"
...
echo "[run_il_mp3d_1scene_dinov2_cached_252_oracle_object_cloud] mode=${MODE} updates=${NUM_UPDATES} envs=${NUM_ENVIRONMENTS} ckpts=${NUM_CHECKPOINTS}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_oracle_object_cloud] tb=${TENSORBOARD_DIR}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_oracle_object_cloud] ckpt=${CHECKPOINT_DIR}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_oracle_object_cloud] cache=${CACHE_ROOT}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_oracle_object_cloud] replay_mode=${REPLAY_MODE}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_oracle_object_cloud] object_cloud_cache=${OBJECT_CLOUD_CACHE}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_oracle_object_cloud] cloud max_objects=${MAX_OBJECTS}"
