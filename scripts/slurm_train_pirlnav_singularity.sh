...
# Set these before sbatch or edit them here for your cluster.
SIF="${SIF:-/path/to/pirlnav.sif}"
REPO="${REPO:-/path/to/pirlnav}"
EXTRA_BINDS="${EXTRA_BINDS:-}"

CONFIG="${CONFIG:-configs/experiments/il_objectnav_mp3d.yaml}"
TAG="${TAG:-hpc_mp3d_1scene_ddp}"
...
echo "[pirlnav-slurm] sif=${SIF}"
echo "[pirlnav-slurm] repo=${REPO}"
echo "[pirlnav-slurm] config=${CONFIG}"
echo "[pirlnav-slurm] tag=${TAG}"
echo "[pirlnav-slurm] main=${MAIN_ADDR}:${MAIN_PORT}"
echo "[pirlnav-slurm] tasks=${SLURM_NTASKS:-unknown} gpus_per_node=${SLURM_GPUS_ON_NODE:-unknown}"
