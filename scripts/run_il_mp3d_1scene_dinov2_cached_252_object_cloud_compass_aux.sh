...
# sensor (accumulated from live depth + semantic) and the goal-compass auxiliary
# regression loss on the point-transformer's 12-D head.
#
# Drop-in successor to run_il_mp3d_1scene_dinov2_object_cloud_compass_aux.sh:
# same policy inputs (cached DINOv2, online object cloud, compass aux label)
# but uses the pose-replay 252 cache and IL.BehaviorCloning.REPLAY_MODE=poses
# so rollout collection teleports to recorded agent_state each step.
#
...
#   - GOAL_COMPASS_SENSOR (privileged label for auxiliary MSE only)
...
#
# Usage:
#   bash scripts/run_il_mp3d_1scene_dinov2_cached_252_object_cloud_compass_aux.sh           # smoke (200 updates, 2 envs)
#   bash scripts/run_il_mp3d_1scene_dinov2_cached_252_object_cloud_compass_aux.sh --full    # 20k updates, 4 envs
#   NUM_UPDATES=1000 NUM_ENVIRONMENTS=2 \
#       bash scripts/run_il_mp3d_1scene_dinov2_cached_252_object_cloud_compass_aux.sh
#
...
TAG="${TAG:-mp3d_1scene_6cat_dinov2_cached_252_object_cloud_compass_aux}"
TENSORBOARD_DIR="tb/objectnav_il/${TAG}/"
CHECKPOINT_DIR="data/new_checkpoints_dinov2_cached_252_object_cloud_compass_aux/objectnav_il/${TAG}/"
...
echo "[run_il_mp3d_1scene_dinov2_cached_252_object_cloud_compass_aux] mode=${MODE} updates=${NUM_UPDATES} envs=${NUM_ENVIRONMENTS} ckpts=${NUM_CHECKPOINTS}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_object_cloud_compass_aux] tb=${TENSORBOARD_DIR}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_object_cloud_compass_aux] ckpt=${CHECKPOINT_DIR}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_object_cloud_compass_aux] cache=${CACHE_ROOT}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_object_cloud_compass_aux] replay_mode=${REPLAY_MODE}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_object_cloud_compass_aux] cloud max_objects=${MAX_OBJECTS} min_mask_pixels=${MIN_MASK_PIXELS}"
echo "[run_il_mp3d_1scene_dinov2_cached_252_object_cloud_compass_aux] compass_aux_coef=${COMPASS_AUX_COEF}"
