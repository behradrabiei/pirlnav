...
class RNNStateEncoder(nn.Module):
    ...
    def single_forward(
        ...
        # PyTorch >= 2.x's cuDNN GRU kernel rejects non-contiguous hx; the
        # `torch.where` above + any prior transposes can leave a non-contig
        # view.  Make it contiguous to match the kernel's expectations.
        packed_hidden = self.unpack_hidden(hidden_states)
        if isinstance(packed_hidden, tuple):
            packed_hidden = tuple(h.contiguous() for h in packed_hidden)
        else:
            packed_hidden = packed_hidden.contiguous()

        x, hidden_states = self.rnn(
            x.unsqueeze(0), packed_hidden
        )
    ...
    def seq_forward(
        ...
        if isinstance(packed_hidden, tuple):
            ...
            packed_hidden = packed_hidden.contiguous()
        x_seq, hidden_states = self.rnn(x_seq, packed_hidden)
        hidden_states = self.pack_hidden(hidden_states)
