...
class BaseRLTrainer(BaseTrainer):
    ...
    def should_checkpoint(self) -> bool:
        ...
        if self.config.NUM_CHECKPOINTS != -1:
            ...
            if self._last_checkpoint_percent + checkpoint_every < pct:
                needs_checkpoint = True
                self._last_checkpoint_percent = pct
            # Also guarantee a save on the final update.  With the strict `<`
            # above, percent_done == 1.0 never beats last_checkpoint (=0.9 for
            # NUM_CHECKPOINTS=10) by a full interval, so the true end-of-run
            # weights were being lost.  This adds one final save at the end.
            elif self.is_done() and self._last_checkpoint_percent < pct:
                needs_checkpoint = True
                self._last_checkpoint_percent = pct
        else:
            needs_checkpoint = (
