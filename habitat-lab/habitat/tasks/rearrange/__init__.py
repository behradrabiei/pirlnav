...
def _try_register_rearrange_task():
    try:
        ...
        # Rearrange task requires APIs from habitat-sim that changed between
        # 0.2.2 and 0.2.5 (e.g. habitat_sim.robots). pirlnav only uses ObjectNav,
        # so silently skip rearrange registration if those imports fail.
        return
